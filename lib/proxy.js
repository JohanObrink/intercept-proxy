...
ProxyServer.prototype.addFolder = function(urlPath, folderPath) {
  
  // remove folder first
  this.removeFolder(urlPath);
...
ProxyServer.prototype.removeFolder = function(urlPath) {
  
  var existing = this.folders.filter(function(folder) { return folder.path == urlPath });
  
  for(var i=0; i<existing.length; i++) {
    var index = this.folders.indexOf(existing[i]);
