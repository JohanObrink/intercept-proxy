...
var url = require('url');
var autoheaders = require('./headers');
...
var resolve = exports.resolve = function(req, options) {
  var supressQuery = (options && options.supressQuery);
...
var getHeaders = exports.getHeaders = function(filePath, callback) {
  ...
  var headers = _.clone(autoheaders.getHeaders(filePath));

  if(!existsSync(jsonPath)) {
    callback(200, headers);
  } else {
    fs.readFile(jsonPath, function(err, data) {
      var fileHeaders = JSON.parse(data);
      headers = _.extend(headers, fileHeaders);
      callback(200, headers);
    });
  }
};
